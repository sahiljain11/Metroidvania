...
class PoorManPlatformerPhysics
  ...
  def render
    ...
    #Player green box
    outputs.primitives << [state.x,
                           state.y,
                           state.tile_size,
                           state.tile_size,  0, 180, 0].border

    #Label in top left of the screen
    outputs.primitives << [963, 700, 'Press \'s\' to access sprites', 1, 0, 255, 255, 255, 255].label
    outputs.primitives << [920, 675, 'Click existing sprite to delete', 1, 0, 255, 255, 255, 255].label

    #If player hits 's', following appears
    if state.selectMenu[1] != 720
      #White background for sprite selection
      outputs.primitives << [state.selectMenu[0], state.selectMenu[1], state.selectMenu[2], state.selectMenu[3], 255, 255, 255].solid

      #Select tile label at the top of the screen
      outputs.primitives << [state.bannerCords[0], state.bannerCords[1], "Select Sprite", 10, 0, 0, 0, 0, 255].label

      #Places sprites in locations calculated in the defaults function
      outputs.primitives << state.spriteCords.map do
        |x, y, order|
        [x, y, state.spriteSize, state.spriteSize, 'sprites/image' + order.to_s + ".png"].sprite
      end
    end
    ...
    outputs.primitives << [inputs.mouse.position.x - 10, inputs.mouse.position.y,
                           10, 10, 'sprites/image' + state.spriteSelected.to_s + ".png"].sprite
  end
